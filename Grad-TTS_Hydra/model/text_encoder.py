...
class TextEncoder(BaseModule):
    def __init__(self, cfg):
        ...
        self.encoder = Encoder(self.n_channels + (self.spk_emb_dim if self.n_spks > 1 else 0), self.filter_channels, self.n_heads, self.n_layers, 
                               self.kernel_size, self.p_dropout, window_size=self.window_size)

        self.proj_m = torch.nn.Conv1d(self.n_channels + (self.spk_emb_dim if self.n_spks > 1 else 0), self.n_feats, 1)
        self.proj_w = DurationPredictor(self.n_channels + (self.spk_emb_dim if self.n_spks > 1 else 0), self.filter_channels_dp, 
                                        self.kernel_size, self.p_dropout)
