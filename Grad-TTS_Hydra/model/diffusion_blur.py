...
class Diffusion(BaseModule):
    ...
    def forward_plot(self, X0, mask, mu, a):
        device = X0.device
        n_steps = self.n_timesteps
        
        n_all = torch.arange(0, n_steps + 1, dtype=torch.int64, device=device)
        alpha_all = 1 - (1 - a) / n_steps * n_all  #a_n ranges from 1 to 0.1
        
        mu_11 = mu[0].unsqueeze(0).repeat(n_steps + 1, 1, 1)
        X0_11 = X0[0].unsqueeze(0).repeat(n_steps + 1, 1, 1)

        n_all = n_all.view(-1, 1, 1)
        Xn_forward = self.forward_diffusion(X0_11, mask, mu_11, n_all ,a)

        os.makedirs(f'forward_pass', exist_ok=True)

        for i, mel_spec in enumerate(Xn_forward):
            pt_to_pdf(mel_spec.cpu(), f'forward_pass/X_{i}.pdf' , vmin=-12.5, vmax=0.0)
