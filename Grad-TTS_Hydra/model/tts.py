...
class GradTTS(BaseModule):
    def __init__(self, cfg):
        ...
        self.spk_emb = torch.nn.Embedding(self.n_spks, self.spk_emb_dim) if self.n_spks > 1 else None
        
        self.encoder = TextEncoder(cfg)
        self.decoder = Diffusion(cfg)
        self.train_encoder = cfg.training.train_encoder
    @torch.no_grad()
    def forward(self, x, x_lengths, n_timesteps, temperature=1.0, stoc=False, spk=None, length_scale=1.0, output_dir = 'save_2'):
        """
        ...
        decoder_outputs = self.decoder(mu_y, y_mask, mu_y, n_timesteps, stoc, spk)
